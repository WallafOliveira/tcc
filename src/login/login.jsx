...
function Login() {
  ...
  const [valida, setValida] = useState(
    {
      email: "",
      password: "",
      errors: {email: "Email is necessary", password: "Password is necessary", },
      emailRequirements: {hasAtSymbol: false, validFormat: false, },
      passwordRequirements: { length: false, number: false, specialCharacter: false, uppercaseLetter: false, },
    }
  );
  function validateForm() {
    ...
    const emailRequirements = {hasAtSymbol, validFormat, };
    // Password validation
    const passwordRequirements = {length: password.length >= 8, number: /\d/.test(password), specialCharacter: /[!@#$%^&*]/.test(password), uppercaseLetter: /[A-Z]/.test(password), };
    if (password) {
      errors.password = "";
    }
    else {
      ...
      <div className="container_login">
        ...
        <div className="email_password">
          ...
          <div className="divEmail">
            <label>Email:</label>
            <input className={
              `entrada ${valida.errors.email ? "invalid" : valida.emailRequirements.validFormat ? "valid" : "" }`
            } type="email" placeholder="email@gmail.com" name="email" value={valida.email} onChange={e => handleInputChange(e)} />
            <div className={
              `error ${valida.errors.email ? "invalid" : ""}`
            } />
            <ul className="email-requirements">
              <li className={
                  `email-requirement ${valida.emailRequirements.hasAtSymbol ? "valid" : "invalid"}`
              } > Email deve conter "@" </li>
              <li className={
                  `email-requirement ${valida.emailRequirements.validFormat ? "valid" : "invalid"}`
          ...
          <div>
            ...
            } type="password" placeholder="123ABC!" name="password" value={valida.password} onChange={e => handleInputChange(e)} />
            <div className={
              `error ${valida.errors.password ? "invalid" : ""}`
            } />
            <ul className="password-requirements">
              ...
              }`} > Precisa ter números </li>
              <li className={`password-requirement ${
                  valida.passwordRequirements.specialCharacter ? "valid" : "invalid"
              }`} > Precisa ter um caracter especial (ex: @#$%) </li>
              <li className={`password-requirement ${
                  valida.passwordRequirements.uppercaseLetter ? "valid" : "invalid"
                }`} > Precisa ter uma letra maiúscula </li>
            </ul>
          </div>
        </div>
        {/* FALTA VALIDAR SENHA */}
